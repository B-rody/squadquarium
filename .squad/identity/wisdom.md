...
**Pattern:** `pnpm pack-all` drops the tarball at the repo root (`./squadquarium-*.tgz`), NOT in `packages/cli/` — any doc or script that references `packages/cli/squadquarium-*.tgz` is silently broken; confirmed by live dogfood 2026-05-06. **Context:** README Quick-start, copilot-instructions pack-install-smoke snippet, any CI step that installs the packed tarball.